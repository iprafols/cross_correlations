# 2016.07.26 12:10:08 CEST
#Embedded file name: /Users/iprafols/software/development/cross_correlations/lib/correlation_process.py
"""
Module to process the outputs of the cross-correlation code

DESCRIPTION:
This module provides the classes CorrData, CorrDataError, and CorrDataWarning 
to enable easy treatment of the outputs of the cross-correlation code. The first 
class is for data treatment and the others are for handling errors and warnings.
...
class CorrModel(object):
    ...
    def rebinData(self, transformation_matrix, keep_results = False, rebin_grid = True):
        ...
        cov_mat_new = np.linalg.inv(inv_cov_mat_new)
        model_mat_new = np.dot(cov_mat_new, np.dot(transformation_matrix.transpose(), self._model_mat))
...
def plot(data_list, fmt_list, save_to, sigma_bins = None, contour = False, plot_rebinned_list = False, labels_list = None, shifts_list = None, save_extension = 'eps'):
    """
    ...
    KEYWORD_ARGUMENTS:
        ...
        contour (bool):                          If True, plots the contour plots instead
                                                 ...
                                                 is ignored, and only one dataset is allowed.
                                                 Labels, if present, are ignored. -- Default: 
                                                 False
    ...
    EXAMPLES:
        plot(data, fmt, "./")
        plot(data, fmt, "./", sigma_bins=sigma_bins)
        plot(data, fmt, "./", contour=True)
        plot([data1, data2], [fmt1, fmt2], "./")
        plot([data1, data2], [fmt1, fmt2], "./", sigma_bins=sigma_bins)
        plot((data1, data2), [fmt1, fmt2], "./")
        plot([data1, data2], [fmt1, fmt2], "./")
        plot([data1, data2], [fmt1, fmt2], "./", plot_rebinned=True)
    ...
    try:
        ...
        print type(fmt_list) == str or type(fmt_list) == list or type(fmt_list) == tuple
        if type(fmt_list) == list or type(fmt_list) == tuple:
            for item in fmt_list:
                print type(item) == str,

            print ''
        print type(save_to) == str
        print sigma_bins == None or isinstance(sigma_bins, np.ndarray)
        print type(contour) == bool
        print type(plot_rebinned_list) == bool or type(plot_rebinned_list) == list or type(plot_rebinned_list) == tuple
        if type(plot_rebinned_list) == list or type(plot_rebinned_list) == tuple:
            for item in plot_rebinned_list:
                print type(item) == bool,

            print ''
        print labels_list == None or type(labels_list) == str or type(labels_list) == list or type(labels_list) == tuple
        if type(labels_list) == list or type(labels_list) == tuple:
            for item in labels_list:
                print type(item) == str,
    ...
    if contour:
        ...
        cs = ax.contourf(sigma_bins, pi_bins, data_averaged, levels, cmap=cmap, vmin=vmin, vmax=vmax, fontsize=labelsize)
        ...
        for bin_num, sigma_min in enumerate(sigma_bins[:-1]):
            ...
            fig.savefig('cross_correlation_sigma_bin_{}.{}'.format(bin_num, save_extension))
            plt.close(fig)
...
def testRebinning(num_test_groups):
    ...
    PURPOSE:
        ...
        cross-correlation output files with the same extension. The file 'test#.grid' my or may
        not be missing.
...
if __name__ == '__main__':
    testRebinning(4)
+++ okay decompyling correlation_process.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2016.07.26 12:10:10 CEST
